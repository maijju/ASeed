...
#include "ASeedEnemyBullet.generated.h"

UCLASS()
class ASEED_API AASeedEnemyBullet : public AActor
{
	GENERATED_BODY()
...
protected:
	...
	/*--------PRIMARY DATA--------*/
	UPROPERTY()
	const UASeedEnemyBulletData* BulletData;

	/*--------LOCAL VARIABLES--------*/
	class AController* OwnerController;
	UPROPERTY(EditAnywhere)
	float LifeDuration = 3;
	float Damage = 0;
...
public:
	...
	void SetBulletData(const UASeedEnemyBulletData* Data)
	{
		BulletData = Data;
		MoveComp->Velocity = GetActorForwardVector() * BulletData->BulletSpeed;
		MeshComp->SetStaticMesh(BulletData->Mesh);
		ApplyTrailEffect();
	}
